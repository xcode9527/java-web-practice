...
~~~shell
[client]
socket=/opt/backend/mysql/manage/tmp/mysql.sock

[mysqld_safe]
log-error=/opt/backend/mysql/manage/log/mysql.err 
pid-file=/opt/backend/mysql/manage/var/mysql.pid
...
# 数据存储目录
datadir=/opt/backend/mysql/manage/data/
...
#错误链接次数,默认10,超过以后被屏蔽,可以防止dos攻击
max_connect_errors = 1000

#事务隔离级别默认为可重复读,Oracle是读已提交
...
#不为1的话TIMESTAMP类型和其他的类型不一样,没仔细研究
...
#开启通用日志,会纪录所有操作,但是严重影响性能,一般没特殊要求也没人打开
general_log = 0
